    ...
    protected function forgottenPasswordSubmit(Request $request) {
        ...
        if($api_response->success) {
            ...
            Mail::send(array(), array(), function($message) use ($body, $data) {
                $message->to($data['email'])->subject('Dentacare Dentacoin - Request for password change');
                $message->from(EMAIL_SENDER, 'Dentacare Dentacoin')->replyTo(EMAIL_SENDER, 'Dentacare Dentacoin');
